...
export interface SessionBundleExportPlanEntry {
  relativePath: string;
  kind: 'file' | 'directory';
  source: 'copy' | 'filtered_artifact_metadata' | 'filtered_runtime_sqlite';
}
...
async function planSelectedSessionTree(
  ...
  for (const entry of (await readdir(sourcePath, { withFileTypes: true })).sort((a, b) =>
    ...
    if (entry.name === sessionId) {
      ...
      await inspectTree(childPath, `sessions/${sessionId}`, stateRoot, entries, 'copy');
      continue;
    }
...
async function canonicalizeExistingOrMissingRoot(path: string, role: string): Promise<string> {
  ...
  try {
    ...
    const parent = await nearestExistingDirectory(dirname(requestedPath), role);
...
async function nearestExistingDirectory(path: string, role: string): Promise<string> {
