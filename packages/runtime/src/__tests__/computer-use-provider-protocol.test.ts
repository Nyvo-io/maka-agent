...
describe('Kimi OpenAI-compatible product loop', () => {
  test('replays explicit empty reasoning after missing-ledger recovery and backend recreation', async () => {
    ...
    const replayedAssistants = (requestBodies[1]!.messages as unknown[]).filter(
      (message) => isRecord(message) && message.role === 'assistant',
    );
    assert.equal(
      replayedAssistants.length,
      1,
      `recovered text and reasoning must replay as one assistant message: ${JSON.stringify(requestBodies[1]!.messages)}`,
    );
    const replayedAssistant = replayedAssistants.find(
      (message) =>
        isRecord(message) &&
    ...
    assert.equal(replayedAssistant.content, 'turn-1-done');
