...
describe('AiSdkBackend RunTrace', () => {
  ...
  test('does not persist empty reasoning metadata from a failed provider attempt', async () => {
    const appended: StoredMessage[] = [];
    let calls = 0;
    const model = new MockLanguageModelV4({
      doStream: async () => {
        ...
        const chunks: LanguageModelV4StreamPart[] =
          calls === 1
            ? [
                ...
                { type: 'reasoning-delta', id: 'failed-reasoning', delta: '' },
                {
                  type: 'error',
                  error: new APICallError({
                    message: 'retry after empty reasoning',
            ...
            : [
                { type: 'stream-start', warnings: [] },
                { type: 'text-start', id: 'successful-text' },
                { type: 'text-delta', id: 'successful-text', delta: 'recovered' },
                { type: 'text-end', id: 'successful-text' },
                {
                  type: 'finish',
                  finishReason: { unified: 'stop', raw: 'stop' },
                  usage: {
    ...
    assert.deepEqual(
      ...
      [{ text: 'recovered', thinking: undefined }],
    );
  });
