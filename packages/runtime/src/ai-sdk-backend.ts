          ...
          for (;;) {
            ...
            if (sawStreamError && !this.aborted) {
              ...
              if (recovered) {
                ...
                // Recovery starts a new provider request. Discard any empty
                // reasoning marker or dialect metadata observed before the
                // failed request reported its overflow.
                resetStepOutput();
              ...
              if (
                ...
                // An explicit empty reasoning delta is safe to retry, but its
                // provider dialect still belongs to the failed request. Start
                // the next request with a clean step accumulator so that
                // metadata cannot leak into the successful assistant message.
                resetStepOutput();
