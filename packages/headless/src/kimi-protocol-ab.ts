...
export function recommendKimiProtocolDefault(input: {
  ...
  const improvesLatency =
    ...
    materiallyImproves(input.openai.totalLatencyMs, input.anthropic.totalLatencyMs);
  const improvesCost =
...
function materiallyImproves(candidate: number, baseline: number): boolean {
  return (
    baseline > 0 &&
    candidate < baseline &&
    (baseline - candidate) / baseline >= MIN_DEFAULT_CHANGE_IMPROVEMENT_RATIO
  );
}
